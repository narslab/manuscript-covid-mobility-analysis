...
\section{Data}
We use Community Mobility Reports compiled by Google\footnote{\url{https://www.google.com/covid19/mobility/}}. %The data is publicly available at:
...
We employed unsupervised learning methods to visualize and explore the time series.
First, we used the dynamic time warping (DTW) algorithm \trbcite{giorgino2009computing} to compute the dissimilarity matrix for the pairwise distances between the multivariate time series of each country. 
We then applied the Ward criterion \trbcite{murtagh2014ward} for hierarchical clustering to group the countries across the prevailing patterns in the dataset. The dendogram of the four clusters extracted is shown in \autoref{fig:dend}.
